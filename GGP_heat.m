...
function GGP_heat(nelx, nely, volfrac, method)
%% Problem set-up
...
% nelx=100;nely=100; 
BC='Heat_transfer';%L-shape %Short_Cantilever%MBB
% p.method='MNA';%MMC%MNA %GP this change the function employed for the evaluation of local volume fraction
p.method = method;
% q=3;%q=1
switch method
    case 'MMC'
...
ncx=5; % number of components in the x direction
ncy=5; % number of components in the y direction
...
fixed_dir = ones(size(fixeddofs));
fixednodes = fixeddofs;
excitation_node = fixednodes;
excitation_direction = fixed_dir;
alldofs = [1:ndof];
...
while  stop_cond
    ...
    E(fullelts) = p.E0;
    % FE-ANALYSIS
    sK = reshape(KE(:)*(E(:)'),16*nelx*nely,1);
    K = sparse(iK,jK,sK);
    U(freedofs) = K(freedofs,freedofs)\F(freedofs);
    % OBJECTIVE FUNCTION AND SENSITIVITY ANALYSIS
    ce = reshape(sum((U(edofMat)*KE).*U(edofMat),2),nely,nelx);
    c = sum(sum(E.*(xPhys.^p.penalty).*ce));
%     c = sum(sum((E).*ce));
    v=mean(xPhys(:));
%     dc_dE = -ce;
    dc_dE = -(xPhys.^p.penalty).*ce;
    dc_dE(emptyelts) = 0;
    dc_dE(fullelts) = 0;
    dc_dX=dE_dX*dc_dE(:);
    dc_dY=dE_dY*dc_dE(:);
...
%     if rem(outit,plot_rate)==0
        ...
        map=map(end:-1:1,:);
        caxis([0 1])
        patchplot2 = patch('Vertices',[Xx,Yy],'Faces',edofMat,'FaceVertexCData',(1-xPhys(:))*[1 1 1],...
